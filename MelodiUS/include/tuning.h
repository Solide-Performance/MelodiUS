...
inline static void InitNoteLookup()
{
    static bool init = false;
    if(init == false)
    {
        init       = true;
        noteLookup = {std::tuple{82.41, "E2", NoteValue::E2}, {87.31, "F2", NoteValue::F2},
...
{
    /* clang-format off */
    auto it = std::min_element(noteLookup.begin(), noteLookup.end(),
                     [freq](const noteLookup_t& a, const noteLookup_t& b)
                     {
