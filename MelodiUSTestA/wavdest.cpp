...
: CTransformFilter(NAME("WavDest filter"), pUnk, CLSID_WavDest), m_cbWavData(0), m_cbHeader(0)
{
    ASSERT(m_pOutput == 0);
    ASSERT(phr);
...
HRESULT CWavDestFilter::StartStreaming()
{
    ...
    m_cbHeader = sizeof(RIFFLIST) + sizeof(RIFFCHUNK) + m_pInput->CurrentMediaType().FormatLength()
                 + sizeof(RIFFCHUNK);
...
HRESULT CWavDestFilter::StopStreaming()
{
    ...
    if(SUCCEEDED(hr))
    {
        ...
        RIFFCHUNK* pRiffData =
          (RIFFCHUNK*)(((BYTE*)(pRiffFmt + 1)) + m_pInput->CurrentMediaType().FormatLength());
